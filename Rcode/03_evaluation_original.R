...
# saveRDS(pred_error_bic,file = paste("forecasts/","pred_error_bic",".rda",sep = ""))
...
model_list <- c(ar,shrink_poor,rf_poor,bols_poor,bbs_poor,svr_linear_poor,svr_rbf_poor,
                ardi,shrink_rich,rf_rich,bols_rich,bbs_rich,svr_linear_rich,svr_rbf_rich,
                b1,b2,b3)


models_list <- c("ar_aic","ar_cv","shrink_poor_cv_ridge","shrink_poor_cv_lasso","shrink_poor_cv_en","rf_poor_cv","bols_poor_cv","bbs_poor_cv","svr_linear_poor_cv","svr_rbf_poor_cv",
                 "ardi_bic","ardi_aic","ardi_cv","shrink_rich_cv_ridge","shrink_rich_cv_lasso","shrink_rich_cv_en","rf_rich_cv","bols_rich_cv","bbs_rich_cv","svr_linear_rich_cv","svr_rbf_rich_cv",
                 "b1_cv_boost","b1_cv_ridge","b1_cv_en","b1_cv_lasso","b2_cv_boost","b2_cv_ridge","b2_cv_en","b2_cv_lasso","b3_cv_boost","b3_cv_ridge","b3_cv_en","b3_cv_lasso")

ar_bic_rmspe <- readRDS("forecasts/ar_bic_results.rda")
pred_error_bic <- readRDS("forecasts/pred_error_bic.rda")
matrix_list <- list()
CSFE <- list()
CSFE_list <- list()
...
columns_to_exclude <- c("shrink_poor_cv_lasso","shrink_poor_cv_en","b1_cv_en","b2_cv_lasso", "b3_cv_lasso", "b2_cv_en", "b3_cv_en")
...
columns_to_exclude <- c("shrink_poor_cv_lasso","b2_cv_lasso", "b3_cv_lasso", "b2_cv_en")
...
IPCA_1_CSFE_df <- IPCA_1_CSFE %>%
  as.data.frame()%>%
  mutate(dates = seq(as.Date("2009-01-01"), by = "month", length.out = 125))%>%
  select(b1_cv_lasso,b3_cv_lasso,b2_cv_lasso,b2_cv_en,b2_cv_boost,b3_cv_ridge,b2_cv_ridge,b1_cv_ridge,svr_rbf_rich_cv,b1_cv_en,dates) %>%
  gather(key = "Model", value = "value", -dates)
IPCA_3_CSFE_df <- IPCA_3_CSFE %>%
  as.data.frame()%>%
  mutate(dates = seq(as.Date("2009-01-01"), by = "month", length.out = 125))%>%
  select(b3_cv_boost,rf_rich_cv,b1_cv_boost,b2_cv_boost,b3_cv_ridge,b3_cv_en,b1_cv_en,b1_cv_lasso,b2_cv_ridge,bbs_rich_cv,dates) %>%
  gather(key = "Model", value = "value", -dates)
IPCA_6_CSFE_df <- IPCA_6_CSFE %>%
  as.data.frame()%>%
  mutate(dates = seq(as.Date("2009-01-01"), by = "month", length.out = 125))%>%
  select(b3_cv_en,b1_cv_lasso,b1_cv_en,b2_cv_boost,svr_rbf_rich_cv,b3_cv_boost,bbs_rich_cv,ar_cv,b1_cv_boost,bols_rich_cv,dates) %>%
  gather(key = "Model", value = "value", -dates)
IPCA_9_CSFE_df <- IPCA_9_CSFE %>%
  as.data.frame()%>%
  mutate(dates = seq(as.Date("2009-01-01"), by = "month", length.out = 125))%>%
  select(b1_cv_lasso,b3_cv_en,b1_cv_en,b2_cv_boost,svr_rbf_rich_cv,b3_cv_boost,ar_cv,bbs_rich_cv,svr_rbf_poor_cv,bols_rich_cv,dates) %>%
  gather(key = "Model", value = "value", -dates)
IPCA_12_CSFE_df <- IPCA_12_CSFE %>%
  as.data.frame()%>%
  mutate(dates = seq(as.Date("2009-01-01"), by = "month", length.out = 125))%>%
  select(b1_cv_lasso,b1_cv_en,b3_cv_en,b2_cv_boost,b3_cv_boost,svr_rbf_rich_cv,bols_rich_cv,shrink_rich_cv_ridge,svr_rbf_poor_cv,bbs_rich_cv,dates) %>%
  gather(key = "Model", value = "value", -dates)
...
UNRATE_1_CSFE_df <- UNRATE_1_CSFE %>%
  as.data.frame()%>%
  mutate(dates = seq(as.Date("2009-01-01"), by = "month", length.out = 125))%>%
  select(rf_rich_cv,svr_rbf_rich_cv,b1_cv_en,b1_cv_lasso,b1_cv_ridge,b2_cv_en,b2_cv_lasso,b3_cv_ridge,b3_cv_en,b3_cv_lasso,dates) %>%
  gather(key = "Model", value = "value", -dates)
UNRATE_3_CSFE_df <- UNRATE_3_CSFE %>%
  as.data.frame()%>%
  mutate(dates = seq(as.Date("2009-01-01"), by = "month", length.out = 125))%>%
  select(b3_cv_en,b1_cv_lasso,b1_cv_en,b3_cv_lasso,b2_cv_en,b3_cv_ridge,b2_cv_lasso,b2_cv_ridge,svr_rbf_rich_cv,rf_rich_cv,dates) %>%
  gather(key = "Model", value = "value", -dates)
UNRATE_6_CSFE_df <- UNRATE_6_CSFE %>%
  as.data.frame()%>%
  mutate(dates = seq(as.Date("2009-01-01"), by = "month", length.out = 125))%>%
  select(b3_cv_ridge,b1_cv_en,b2_cv_ridge,b3_cv_en,b1_cv_lasso,b2_cv_en,svr_rbf_rich_cv,shrink_poor_cv_ridge,b2_cv_boost,b3_cv_boost,dates) %>%
  gather(key = "Model", value = "value", -dates)
UNRATE_9_CSFE_df <- UNRATE_9_CSFE %>%
  as.data.frame()%>%
  mutate(dates = seq(as.Date("2009-01-01"), by = "month", length.out = 125))%>%
  select(svr_rbf_rich_cv,b1_cv_en,b3_cv_en,b1_cv_lasso,b2_cv_en,b3_cv_ridge,b2_cv_ridge,shrink_poor_cv_ridge,bbs_poor_cv,rf_rich_cv,dates) %>%
  gather(key = "Model", value = "value", -dates)
