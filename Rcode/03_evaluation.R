...
# ###########################
# model_list <- readRDS("forecasts/ar_bic.rda")
# for (v in variable) {
#   for (h in horizon_list) {
#     filtered_models <- model_list[sapply(model_list, function(x) x$horizon == h & x$variable == v & x$model == "ar_bic_bic")]
...
# print(rmspe_data)
# saveRDS(rmspe_data,file = paste("forecasts/","ar_bic_results",".rda",sep = ""))
...
# Data poor
ar <- readRDS("forecasts/ar.rda")
shrink_poor <- readRDS("forecasts/shrink_poor.rda")
shrink_poor_en <- readRDS("forecasts/shrink_poor_en.rda")
rf_poor <- readRDS("forecasts/rf_poor.rda")
bols_poor <- readRDS("forecasts/bols_poor.rda")
bbs_poor <- readRDS("forecasts/bbs_poor.rda")
svr_linear_poor <- readRDS("forecasts/svr_linear_poor.rda")
svr_rbf_poor <- readRDS("forecasts/svr_rbf_poor.rda")

# Data rich
ardi <- readRDS("forecasts/ardi.rda")
shrink_rich <- readRDS("forecasts/shrink_rich.rda")
shrink_rich_en <- readRDS("forecasts/shrink_rich_en.rda")
rf_rich <- readRDS("forecasts/rf_rich.rda")
bols_rich <- readRDS("forecasts/bols_rich.rda")
bbs_rich <- readRDS("forecasts/bbs_rich.rda")
svr_linear_rich <- readRDS("forecasts/svr_linear_rich.rda")
svr_rbf_rich <- readRDS("forecasts/svr_rbf_rich.rda")

# Bs
b1 <- readRDS("forecasts/b1.rda")
b1_en <- readRDS("forecasts/b1_en.rda")
b2 <- readRDS("forecasts/b2.rda")
b2_en <- readRDS("forecasts/b2_en.rda")
b3 <- readRDS("forecasts/b3.rda")
b3_en <- readRDS("forecasts/b3_en.rda")
...
models_list <- c("ar_aic","ar_cv","shrink_poor_cv_ridge","shrink_poor_cv_lasso","shrink_poor_en_2_cv_en","rf_poor_cv","bols_poor_cv","bbs_poor_cv","svr_linear_poor_cv","svr_rbf_poor_cv",
                 "ardi_bic","ardi_aic","ardi_cv","shrink_rich_cv_ridge","shrink_rich_cv_lasso","shrink_rich_en_2_cv_en","rf_rich_cv","bols_rich_cv","bbs_rich_cv","svr_linear_rich_cv","svr_rbf_rich_cv",
                 "b1_cv_boost","b1_cv_ridge","b1_en_cv_en","b1_cv_lasso","b2_cv_boost","b2_cv_ridge","b2_en_cv_en","b2_cv_lasso","b3_cv_boost","b3_cv_ridge","b3_en_cv_en","b3_cv_lasso")

ar_bic_rmspe <- readRDS("forecasts/ar_bic_results.rda")
model_ar_bic <- readRDS("forecasts/ar_bic.rda")
matrix_list <- list()
CSFE <- list()
CSFE_list <- list()
...
for (v in variable) {
  for (h in horizon_list) {
    for (model in models_list){
      ...
      model_list_filtered_bic <- model_ar_bic[sapply(model_ar_bic, function(x) x$horizon == h & x$variable == v & x$model == "ar_bic_bic")]
...
columns_to_exclude <- c("shrink_poor_cv_lasso","b2_cv_lasso", "b3_cv_lasso")
...
columns_to_exclude <- c("shrink_poor_cv_lasso","b2_cv_lasso", "b3_cv_lasso")
IPCA_3 <- IPCA_3[, -which(colnames(IPCA_3) %in% columns_to_exclude)]
...
columns_to_exclude <- c("shrink_poor_cv_lasso","b3_cv_lasso")
...
# Get 10 best models according to RANK_M of MCS
SPREAD_1_CSFE_df <- SPREAD_1_CSFE %>%
  as.data.frame()%>%
  mutate(dates = seq(as.Date("2009-01-01"), by = "month", length.out = 125))%>%
  select(b1_cv_lasso,b3_cv_boost,svr_rbf_rich_cv,b2_cv_boost,shrink_poor_cv_ridge,ar_cv,b2_cv_ridge,bbs_poor_cv,b3_cv_ridge,rf_poor_cv,dates) %>%
  gather(key = "Model", value = "value", -dates)
SPREAD_3_CSFE_df <- SPREAD_3_CSFE %>%
  as.data.frame()%>%
  mutate(dates = seq(as.Date("2009-01-01"), by = "month", length.out = 125))%>%
  select(svr_linear_poor_cv, b1_cv_lasso,bols_rich_cv,bbs_poor_cv, bols_poor_cv, svr_rbf_rich_cv, bbs_rich_cv, ar_aic, shrink_rich_cv_ridge, svr_linear_rich_cv,dates) %>%
  gather(key = "Model", value = "value", -dates)
SPREAD_6_CSFE_df <- SPREAD_6_CSFE %>%
  as.data.frame()%>%
  mutate(dates = seq(as.Date("2009-01-01"), by = "month", length.out = 125))%>%
  select(b3_cv_boost, b2_cv_boost,svr_linear_poor_cv, b1_cv_lasso, bols_poor_cv, bols_rich_cv,ar_aic,svr_rbf_rich_cv,bbs_rich_cv,shrink_poor_cv_ridge,dates) %>%
  gather(key = "Model", value = "value", -dates)
SPREAD_9_CSFE_df <- SPREAD_9_CSFE %>%
  as.data.frame()%>%
  mutate(dates = seq(as.Date("2009-01-01"), by = "month", length.out = 125))%>%
  select(ar_aic,b3_cv_boost,b2_cv_boost,shrink_poor_cv_ridge,svr_linear_poor_cv,b1_cv_lasso,bols_poor_cv, b1_cv_boost, bbs_rich_cv,svr_linear_rich_cv,dates) %>%
  gather(key = "Model", value = "value", -dates)
SPREAD_12_CSFE_df <- SPREAD_12_CSFE %>%
  as.data.frame()%>%
  mutate(dates = seq(as.Date("2009-01-01"), by = "month", length.out = 125))%>%
  select(svr_linear_poor_cv,bols_poor_cv, shrink_poor_cv_ridge,ar_aic, b1_cv_lasso, bbs_rich_cv, b1_cv_boost, svr_rbf_rich_cv,b2_cv_boost,ar_cv,dates) %>%
  gather(key = "Model", value = "value", -dates)
...
IPCA_1_CSFE_df <- IPCA_1_CSFE %>%
  as.data.frame()%>%
  mutate(dates = seq(as.Date("2009-01-01"), by = "month", length.out = 125))%>%
  select(b1_cv_lasso,b3_cv_lasso,b2_cv_lasso,b2_en_cv_en,b2_cv_boost,b3_cv_ridge,b2_cv_ridge,b1_cv_ridge,svr_rbf_rich_cv,b1_en_cv_en,dates) %>%
  rename(b2_cv_en = b2_en_cv_en) %>%
  rename(b1_cv_en = b1_en_cv_en) %>%
  gather(key = "Model", value = "value", -dates)
IPCA_3_CSFE_df <- IPCA_3_CSFE %>%
  as.data.frame()%>%
  mutate(dates = seq(as.Date("2009-01-01"), by = "month", length.out = 125))%>%
  select(b3_cv_boost,rf_rich_cv,b1_cv_boost,b2_cv_boost,b3_cv_ridge,b3_en_cv_en,b1_en_cv_en,b1_cv_lasso,b2_cv_ridge,bbs_rich_cv,dates) %>%
  rename(b3_cv_en = b3_en_cv_en) %>%
  rename(b1_cv_en = b1_en_cv_en) %>%
  gather(key = "Model", value = "value", -dates)
IPCA_6_CSFE_df <- IPCA_6_CSFE %>%
  as.data.frame()%>%
  mutate(dates = seq(as.Date("2009-01-01"), by = "month", length.out = 125))%>%
  select(b3_en_cv_en,b1_cv_lasso,b1_en_cv_en,b2_cv_boost,svr_rbf_rich_cv,b3_cv_boost,bbs_rich_cv,ar_cv,b1_cv_boost,bols_rich_cv,dates) %>%
  rename(b3_cv_en = b3_en_cv_en) %>%
  rename(b1_cv_en = b1_en_cv_en) %>%
  gather(key = "Model", value = "value", -dates)
IPCA_9_CSFE_df <- IPCA_9_CSFE %>%
  as.data.frame()%>%
  mutate(dates = seq(as.Date("2009-01-01"), by = "month", length.out = 125))%>%
  select(b1_cv_lasso,b3_en_cv_en,b1_en_cv_en,b2_cv_boost,svr_rbf_rich_cv,b3_cv_boost,ar_cv,bbs_rich_cv,svr_rbf_poor_cv,bols_rich_cv,dates) %>%
  rename(b3_cv_en = b3_en_cv_en) %>%
  rename(b1_cv_en = b1_en_cv_en) %>%
  gather(key = "Model", value = "value", -dates)
IPCA_12_CSFE_df <- IPCA_12_CSFE %>%
  as.data.frame()%>%
  mutate(dates = seq(as.Date("2009-01-01"), by = "month", length.out = 125))%>%
  select(b1_cv_lasso,b1_en_cv_en,b3_en_cv_en,b2_cv_boost,b3_cv_boost,svr_rbf_rich_cv,bols_rich_cv,shrink_rich_cv_ridge,svr_rbf_poor_cv,bbs_rich_cv,dates) %>%
  rename(b3_cv_en = b3_en_cv_en) %>%
  rename(b1_cv_en = b1_en_cv_en) %>%
  gather(key = "Model", value = "value", -dates)
...
UNRATE_1_CSFE_df <- UNRATE_1_CSFE %>%
  as.data.frame()%>%
  mutate(dates = seq(as.Date("2009-01-01"), by = "month", length.out = 125))%>%
  select(rf_rich_cv,svr_rbf_rich_cv,b1_en_cv_en,b1_cv_lasso,b1_cv_ridge,b2_en_cv_en,b2_cv_lasso,b3_cv_ridge,b3_en_cv_en,b3_cv_lasso,dates) %>%
  rename(b3_cv_en = b3_en_cv_en) %>%
  rename(b2_cv_en = b2_en_cv_en) %>%
  rename(b1_cv_en = b1_en_cv_en) %>%
  gather(key = "Model", value = "value", -dates)
UNRATE_3_CSFE_df <- UNRATE_3_CSFE %>%
  as.data.frame()%>%
  mutate(dates = seq(as.Date("2009-01-01"), by = "month", length.out = 125))%>%
  select(b3_en_cv_en,b1_cv_lasso,b1_en_cv_en,b3_cv_lasso,b2_en_cv_en,b3_cv_ridge,b2_cv_lasso,b2_cv_ridge,svr_rbf_rich_cv,rf_rich_cv,dates) %>%
  rename(b3_cv_en = b3_en_cv_en) %>%
  rename(b2_cv_en = b2_en_cv_en) %>%
  rename(b1_cv_en = b1_en_cv_en) %>%
  gather(key = "Model", value = "value", -dates)
UNRATE_6_CSFE_df <- UNRATE_6_CSFE %>%
  as.data.frame()%>%
  mutate(dates = seq(as.Date("2009-01-01"), by = "month", length.out = 125))%>%
  select(b3_cv_ridge,b1_en_cv_en,b2_cv_ridge,b3_en_cv_en,b1_cv_lasso,b2_en_cv_en,svr_rbf_rich_cv,shrink_poor_cv_ridge,b2_cv_boost,b3_cv_boost,dates) %>%
  rename(b3_cv_en = b3_en_cv_en) %>%
  rename(b2_cv_en = b2_en_cv_en) %>%
  rename(b1_cv_en = b1_en_cv_en) %>%
  gather(key = "Model", value = "value", -dates)
UNRATE_9_CSFE_df <- UNRATE_9_CSFE %>%
  as.data.frame()%>%
  mutate(dates = seq(as.Date("2009-01-01"), by = "month", length.out = 125))%>%
  select(svr_rbf_rich_cv,b1_en_cv_en,b3_en_cv_en,b1_cv_lasso,b2_en_cv_en,b3_cv_ridge,b2_cv_ridge,shrink_poor_cv_ridge,bbs_poor_cv,rf_rich_cv,dates) %>%
  rename(b3_cv_en = b3_en_cv_en) %>%
  rename(b2_cv_en = b2_en_cv_en) %>%
  rename(b1_cv_en = b1_en_cv_en) %>%
  gather(key = "Model", value = "value", -dates)
UNRATE_12_CSFE_df <- UNRATE_12_CSFE %>%
  as.data.frame()%>%
  mutate(dates = seq(as.Date("2009-01-01"), by = "month", length.out = 125))%>%
  select(b1_cv_lasso,b1_en_cv_en,b3_en_cv_en,b2_en_cv_en,b2_cv_ridge,svr_rbf_rich_cv,b3_cv_ridge,rf_rich_cv,shrink_poor_cv_ridge,bbs_poor_cv,dates) %>%
  rename(b3_cv_en = b3_en_cv_en) %>%
  rename(b2_cv_en = b2_en_cv_en) %>%
  rename(b1_cv_en = b1_en_cv_en) %>%
  gather(key = "Model", value = "value", -dates)
